...
!---------------------------------------------------------------
      ...
      real, intent(in)         :: c3h(nz)                 ! hybrid coordinates
...
!---------------------------------------------------------------
        do j = 1,ny
          ...
          p_wrf(:) = (ps_wrf(1,j) - p_top)*c3h(nz:1:-1) + (znu(nz:1:-1) - c3h(nz:1:-1))*(100000 - p_top) + p_top
...
!---------------------------------------------------------------
        do j = 1, ny
          ...
          p_wrf(:) = (ps_wrf(nx,j) - p_top)*c3h(nz:1:-1) + (znu(nz:1:-1) - c3h(nz:1:-1))*(100000 - p_top) + p_top
          call vinterp( p_moz, p_wrf, wrk, wrk1, nz, nlev )
          do n = 1,nw
...
!---------------------------------------------------------------
        do i = 1,nx
          ...
          p_wrf(:) = (ps_wrf(i,ny) - p_top)*c3h(nz:1:-1) + (znu(nz:1:-1) - c3h(nz:1:-1))*(100000 - p_top) + p_top
          call vinterp( p_moz, p_wrf, wrk, wrk1, nz, nlev )
          do n = 1,nw
...
!---------------------------------------------------------------
        do i = 1,nx
          ...
          p_wrf(:) = (ps_wrf(i,1) - p_top)*c3h(nz:1:-1) + (znu(nz:1:-1)- c3h(nz:1:-1))*(100000 - p_top) + p_top
          call vinterp( p_moz, p_wrf, wrk, wrk1, nz, nlev )
          do n = 1,nw
      ...
      subroutine ic_interpolate4d( ndx, conc, ps_wrf, znu, c3h, p_top, &
                                   nx, ny, nz )
!---------------------------------------------------------------
!     interpolate four-dimensional field ... 
!---------------------------------------------------------------
...
!---------------------------------------------------------------
      ...
      real, intent(in)         :: ps_wrf(nx,ny)           ! wrf surface pressure (Pa)
      real, intent(in)         :: znu(nz)                 ! sigma coordinates
      real, intent(in)         :: c3h(nz)                 ! hybrid cordinates
...
!---------------------------------------------------------------
        do j = 1,ny
          do i = 1,nx
            ...
            p_wrf(:) = (ps_wrf(i,j) - p_top)*c3h(nz:1:-1) + (znu(nz:1:-1) - c3h(nz:1:-1))*(100000 - p_top) + p_top
            call vinterp( p_moz, p_wrf, wrk, wrk1, nz, nlev )
